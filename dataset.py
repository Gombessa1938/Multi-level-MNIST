...
#making downsampled image
data_upsample = torch.zeros(50000,64,64)
for i in tqdm(range(50000)):
  ...
  data_upsample[i] = torch.from_numpy(img_up)

data = data_upsample.numpy()

down_sampled_train = torch.zeros(50000,32,32)
for i in tqdm(range(50000)):
...
difference_train = torch.zeros(50000,64,64)
for i in tqdm(range(50000)):
  ...
  difference_train[i]  = torch.from_numpy(diff)
  
#concat difference image and coarse iamge 
concat_train = torch.zeros(50000,64*64 + 32*32)
for i in tqdm(range(50000)):
...
#-----------------------------------------
down_sampled_train_small = torch.zeros(50000,16,16)
for i in tqdm(range(50000)):
  img = data[i].reshape(64,64)
...
difference_train_medium = torch.zeros(50000,32,32)
for i in tqdm(range(50000)):
  #first = data[i].reshape(64,64)
  ...
  difference_train_medium[i]  = torch.from_numpy(diff)

     
concat_train_medium = torch.zeros(50000,32*32 + 16*16)
for i in tqdm(range(50000)):
  concat_train_medium[i] = torch.cat((difference_train_medium[i].reshape(32*32,),\
    down_sampled_train_small[i].reshape(16*16,))) 
 
concat_train_large = torch.zeros(50000,64*64+32*32+16*16)
for i in tqdm(range(50000)):
  concat_train_large[i] = torch.cat((difference_train[i].reshape(64*64,),concat_train_medium[i].reshape(32*32+16*16,))) 


np.save('concat_train_medium_64_no_diff.npy',concat_train_medium)
np.save('down_sampled_train_small_64_no_diff.npy',down_sampled_train_small)
np.save('concat_train_large_64_no_diff.npy',concat_train_large)